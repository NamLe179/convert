...
export type TreeItem = {
  ...
  isOpen?: boolean; 
  children?: TreeItem[];
  // mở rộng thêm các trường nếu cần 
  [key: string]: any; 
};
    ...
    return data.flatMap((item) => {
      if (item.id === targetId) {
        ...
        return {
          ...
          isOpen: true, // Bây giờ hợp lệ vì đã thêm vào type
  ...
  if (action.type === "instruction") {
    ...
    if (instruction.type === "reparent") {
      ...
      if (!path) {
        ...
        return data; // Nên return data thay vì void để tránh crash UI
      }
...
export function getChildItems(data: TreeItem[], targetId: string) {
